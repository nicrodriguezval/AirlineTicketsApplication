...
public class CalcularPrecio {
    ...
    public double calcularPrecio(Reserva reserva) { //Acá falta completar porque no sé cómo calcularemos el precio del viaje teniendo en cuenta si afora equipaje o si el viaje es ida y vuelta, o ambos
        if(reserva.isIdaVuelta() && reserva.getCategoria().equals("Primera clase")) {
            return precioEquipaje(reserva.getPeso()) + 
                   reserva.getVueloIda().getPrecioPrimeraClase() + 
                   precioEquipaje(reserva.getPesoVuelta()) + 
                   reserva.getVueloVuelta().getPrecioPrimeraClase();
        }
            ...
            return precioEquipaje(reserva.getPeso()) + 
                   reserva.getVueloIda().getPrecioClaseEjecutiva() + 
                   precioEquipaje(reserva.getPesoVuelta()) + 
                   reserva.getVueloVuelta().getPrecioClaseEjecutiva();
        }
            ...
            return precioEquipaje(reserva.getPeso()) + 
                   reserva.getVueloIda().getPrecioClaseTurista() + 
                   precioEquipaje(reserva.getPesoVuelta()) + 
                   reserva.getVueloVuelta().getPrecioClaseTurista();
        }
